...
- **CV Creatorのマテリアルbaselineは二重管理**: `_originalMaterials`（UI差分計算とPrefab asset側baseline表示用）と`_previewOriginalMaterials`（scene復元用）の2つを持つ。pre-existing overrideがあるslotではこの2つが異なる値を持ち、前者はPrefab asset側のmaterial、後者はscan時点のHierarchy instance materialとなる。`SetRendererMaterial(slot, null)`（個別 × / Bulk Clear / no-op回避）は前者を使い「UI上のoverrideをbaseに戻す」操作として動く。`ResetPreview` / `RestoreOriginalMaterials` は後者を使い「scan時点のscene状態へ戻す」操作として動く。Clear All（`OnClearOverrides`）は後者で復元したあとに `SyncOverridesFromRenderers` を呼んで pre-existing override を `_overrides` に再登録し、UIと実体を一致させる。
- **rescan時のbaseline carry-overは既存slot優先**: `ScanMaterialSlotsInternal(preserveOverrides: true)` でのrescan時、既存slotの `_originalMaterials` / `_previewOriginalMaterials` は前回scanから引き継ぎ、追加された新規slotのみ発見時点のmaterialをbaselineにする。Material BrowserのrefreshやHierarchy構造変更でrescanが走っても、既存slotに対する外部編集（Inspector / Scene drag-drop）はbaselineへ吸収せず、`SyncOverridesFromRenderers` によりoverride差分として扱う。一方、`isToolMadeOverride` 判定は `wasPreviewActive` でガードし、preview非アクティブ時にSync経由で `_overrides` に入った外部編集をtool-madeと誤分類してSelective Revertの対象にしてしまうことを防ぐ。
- **Base Prefab fieldの検証はnearest instance root**: `PrefabUtility.GetNearestPrefabInstanceRoot(_baseInstance) == _baseInstance` で「選択がPrefab instance rootか」を判定する。`IsOutermostPrefabInstanceRoot` ではNested Prefab rootが拒否されてしまうため使わない。`_basePrefabAsset` も `GetCorrespondingObjectFromSource` ではなく `LoadAssetAtPath(GetPrefabAssetPathOfNearestInstanceRoot(_baseInstance))` で導出し、Nested Prefab rootを起点とした場合はNested Prefab assetを親とするVariant生成になる。
- **Standardモードのancestor retarget検証はpath比較**: `PrefabVariantGenerator.GenerateStandardVariant` 内の親一致チェックは `GetPrefabAssetPathOfNearestInstanceRoot(hierarchyInstance)` と `AssetDatabase.GetAssetPath(basePrefabAsset)` のpath比較で行う。UI側の`_basePrefabAsset`取得方法と同じ概念に揃え、Nested Prefab rootを起点とした正常generationが`GetCorrespondingObjectFromSource`の戻り値差で誤拒否されないようにする。
