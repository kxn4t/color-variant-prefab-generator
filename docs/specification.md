...
スキャンされた全Rendererのマテリアルスロットを表示。**通常モード**と**一括モード**の2つの表示モードを持ち、セクションヘッダー右のトグルボタン `[通常 | 一括]` で切り替える。切り替えてもデータは変更されない（表示の切り替えのみ）。状態はウィンドウを閉じるとリセットされる。

両モードとも、グループヘッダーは共通の折りたたみコンポーネントを使用。折りたたみ矢印（▶/▼）のクリックで展開/折りたたみを切り替え、Alt+クリックで全グループを一括展開/折りたたみする。ヘッダーコンテンツのクリックは各モード固有の動作を行う。

#### 通常モード

Rendererごとにグループ化して表示（デフォルト）。各Rendererグループは折りたたみ可能（デフォルト展開）。
...
**各スロット行の構成**:
- **左側**（読み取り専用）: ベースマテリアルのサムネイル + 名前（固定幅120px、長い名前は省略表示）
...
- **折りたたみ矢印クリック**: 展開/折りたたみ切り替え（Alt+クリックで全グループ一括）
- **スロット行クリック**: ベースマテリアルをInspectorで選択。ブラウザーに該当マテリアルが存在する場合は自動スクロール＋5秒間ハイライト。クリックしたスロット行自体も青い左ボーダーで5秒間ハイライトされる。連続クリック時は前のハイライトを即座にクリアする
- **オーバーライドサムネイルクリック**: オーバーライドマテリアルをInspectorで選択（同様にブラウザーでハイライト＋スロット行ハイライト）
- **ドラッグ＆ドロップ**: 行全体がマテリアルのドロップターゲット（ブラウザーまたはProjectウィンドウから）

#### 一括モード

同じマテリアルを使用している全スロットをマテリアルごとにグルーピングし、一括でマテリアルを置き換える。グルーピングは**実効マテリアル**（オーバーライドがあればオーバーライド、なければベース）に基づく。
...
│     Body / Slot 2        (base: Material_A)              [×]  │ ← オーバーライドあり
│     Accessory / Slot 0   (base: Material_A)              [×]  │ ← オーバーライドあり
│     Pants / Slot 1                                            │ ← ベース一致(薄表示)
│     Shoes / Slot 0                                            │ ← ベース一致(薄表示)
│                                                               │
...
**グループヘッダー（親行）**:
- マテリアルサムネイル（20×20）+ マテリアル名（太字、固定幅130px、長い名前は省略表示、ツールチップでフル名を表示）+ カウント "(N slots)" + オーバーライド数バッジ（pill型、オーバーライドがある場合のみ表示）+ → + ObjectField + クリアボタン（×）
- オーバーライドが0件のグループ: ヘッダー全体が`opacity: 0.5`、クリアボタンは無効化
- nullマテリアルは「(None)」グループとして表示

**子行（折りたたみ内容、デフォルト折りたたみ）**:
- オーバーライドあり: `ObjectName / Slot N  (base: 元マテリアル名)  [×]` — 通常不透明度
- オーバーライドなし（ベース一致）: `ObjectName / Slot N` — `opacity: 0.5`

**インタラクション**:

| 操作 | 動作 |
|------|------|
| ヘッダーのサムネイル/名前クリック | マテリアルをInspectorで選択＋ブラウザーでハイライト＋ヘッダー行ハイライト |
| 折りたたみ矢印クリック | 展開/折りたたみ切り替え（Alt+クリックで全グループ一括） |
| 子行クリック | 対象GameObjectをHierarchyで選択 |
| 子行のベースマテリアル名クリック | ベースマテリアルをInspectorで選択＋ブラウザーでハイライト |
| ヘッダーにD&D | グループ内全スロットにオーバーライド適用（1 Undo） |
| ヘッダーのObjectField変更 | D&Dと同じ |
| ヘッダーの[×] | グループ内の全オーバーライドを一括クリア（1 Undo） |
| 子行の[×] | その個別スロットのオーバーライドのみクリア → ベースマテリアルのグループに移動 |

**Undoの挙動**:
- 一括操作（D&D / ヘッダークリア）は1回のCtrl+Zで全体がアンドゥされる
- 個別クリア（子行の×ボタン）は通常のスロット単位Undo
...
- **メインボタン**: 全オーバーライドとバリアント名をクリアし、プレビューをリセット
