...
- **CV Creator** — シーンプレビューを見ながらカラバリを1色ずつ作成。同じマテリアルを使う全スロットの一括置き換えにも対応
...
2. 完成したカラバリを別アバターへ一括展開する（Batch Generator）
   新しいアバター向けのMAやPBなどのセットアップが済んだベースPrefab（例：Airi_HonmeiKnit）を指定し、
...
4. 左パネルにRendererごとのマテリアルスロットが一覧表示される — トグルで**一括**モードに切り替えると、マテリアルごとにグルーピングされ、同じマテリアルを使う全スロットをまとめて置き換えられる
...
CV Creatorには2つの生成モードがあり、ベースPrefabフィールドのオプションメニュー（▼）から切り替えられます。デフォルトは **Standard** モードです。

| モード | 保存される内容 |
|---|---|
| **Standard**（デフォルト） | マテリアルオーバーライドに加えて、Hierarchyインスタンス上のGameObjectレベルの変更（GameObjectの追加・削除、および既存GameObject自体のプロパティ変更：rename・active state・Tag・Layer・StaticEditorFlags等）も保存されます。出力設定の**Transform/コンポーネントの変更を含める**オプションを有効にすると、HierarchyインスタンスをそのままVariantとして保存する経路に切り替わり、Unityが認識するoverride（Transform変更、コンポーネントのプロパティ変更、コンポーネントの追加・削除など）をすべて取り込みます |
| **Strict** | マテリアルオーバーライドのみ。Hierarchyインスタンス上の構造変更やプロパティ変更は一切反映されません |

Standardモードでは、追加したGameObject配下のRendererがスロットUI末尾に専用セクションとして表示され、ベースPrefabに存在しないスロットにもオーバーライドを割り当てられます。

> **ヒント：** 用途に応じたモード選択の目安：
> - **マテリアル変更のみ** — Strictモード。マテリアル以外の変更は保存されないため、以降のベースPrefab編集が全Variantにきれいに伝播します
> - **マテリアル変更＋GameObject追加・削除** — Standardモード（デフォルト、オプションOFF）。影響範囲を最小限に抑えて保存できます
> - **Transformやコンポーネントの変更まで保存** — Standardモード＋「Transform/コンポーネントの変更を含める」をON。Unity上でPrefab Variantとして保存するのと同様にすべての変更が保存されます
...
2つのPrefabを比較する際、以下の5段階の優先度でマテリアルスロットをマッチングします。P1-P4では`slotIndex`と`rendererType`の一致が必須です。P5は同じRenderer型を優先しますが、見つからない場合は型違いにもフォールバックします。

| 優先度 | 戦略 | 説明 |
|---|---|---|
| P1 | 完全パス一致 | 同一の階層パス・オブジェクト名・スロットインデックス・Renderer型 |
| P2 | 同一深度 | 同一の階層深度・オブジェクト名・スロットインデックス・Renderer型 |
| P3 | 名前一致 | 深度を問わず、同一のオブジェクト名とスロットインデックス |
| P4 | 大小文字無視 | 大文字小文字を無視した名前一致とスロットインデックス |
| P5 | 類似名 | 構造的サフィックス除去後の名前一致（例: `Body_01` ≈ `Body_02`）または共通ベース名の一致（例: `Ribbon_blue` ≈ `Ribbon_red`）を複合スコアでランキング |

同じ優先度で複数の候補がある場合は、ベースマテリアル名の一致 → 階層パスの類似度 → 階層深度の近さ → レーベンシュタイン距離の順に判定されます。

### 保存される内容

生成されるPrefab Variantに含まれる内容は、CV Creatorのモードによって変わります。

- **Strictモード** — マテリアルオーバーライドのみ。Transformやコンポーネントの変更など、その他のプロパティは保存されません。マテリアルの変更以外は保存されないため、ベースPrefabの変更（ボーン調整やコンポーネント設定など）がすべてのVariantにきれいに伝播し、Variant間でプロパティが意図せずずれる心配もありません。
- **Standardモード**（デフォルト）— マテリアルオーバーライドに加えて、GameObjectレベルの変更（追加・削除、rename、active state、Tag、Layer等）が保存されます。**Transform/コンポーネントの変更を含める**オプションをONにすると、HierarchyインスタンスをそのままVariantとして保存し、Unityが認識するoverrideをすべて保持します。そのため、意図しない変更が紛れる可能性があります。

**Batch Generator**は、CV Creatorのモード設定にかかわらず、常にマテリアルのみのVariantを生成します。
