...
- 更新ボタン: マテリアル一覧とスロットUIを再読み込みする。既存スロットのマテリアルbaselineは変更せず、未反映だった追加Rendererスロットのみ発見時点のマテリアルを新規baselineとして登録する。ツール外で変更された既存スロットのマテリアルはbaselineに吸収せず、override差分として扱う
...
| **Full Revert** | 変更対象の全Rendererのオーバーライドをすべてrevert。ツール使用前から存在していたオーバーライドも対象。 |
