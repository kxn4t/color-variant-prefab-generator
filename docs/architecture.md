...
- **rescan時のbaseline carry-overは active preview 中に限定**: `ScanMaterialSlotsInternal(preserveOverrides: true)` でのrescan時、`_originalMaterials` / `_previewOriginalMaterials` を前回scanから引き継ぐかどうかは `wasPreviewActive` でガードする。preview非アクティブ時は最新のscene状態を新baselineとして信用し、外部編集（Inspector / Scene drag-drop）を保護する。preview中のrescanでのみ、前回baselineを凍結してpreview material混入を防ぐ。同じガードが `isToolMadeOverride` 判定にも適用され、preview非アクティブ時のSync経由で `_overrides` に入った外部編集をtool-madeと誤分類してSelective Revertの対象にしてしまうことを防ぐ。
