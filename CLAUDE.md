...
- **VariantAnalyzer** — 既存Prefab Variantを解析し、ベース名のトークンを除去してバリアント名を導出（`DeriveVariantName` / `StripCommonAffixTokens`）
...
- rescan（`ScanMaterialSlotsInternal(preserveOverrides: true)`）では既存slotの`_originalMaterials` / `_previewOriginalMaterials`を前回scanから引き継ぎ、新規追加slotだけ発見時点のmaterialをbaselineにする。Material BrowserのrefreshやHierarchy構造変更でrescanが走っても、既存slotの外部編集（Inspector / Scene drag-drop）はbaselineに吸収せずoverride差分として扱う。一方、`isToolMadeOverride`分類は`_previewActive`をrescan前に保存した`wasPreviewActive`で必ずガードする。これを外すとSync経由で取り込まれた外部編集をtool-madeと誤分類してSelective Revertが破壊する
